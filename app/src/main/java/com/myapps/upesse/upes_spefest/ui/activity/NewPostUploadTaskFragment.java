...
public class NewPostUploadTaskFragment extends Fragment {
    ...
    /*
    public void resizeBitmap(int resId, int maxDimension) {
        LoadResizedBitmapTask task = new LoadResizedBitmapTask(maxDimension);
        task.execute(resId);
    }

    class LoadResizedBitmapTask extends AsyncTask<Integer, Void, Bitmap> {
        private int mMaxDimension;
        ...
        protected Bitmap doInBackground(Integer... integers) {
            ...
            if (resId != 0) {
                ...
                Bitmap bitmap = null;
                try {
                    bitmap = decodeSampledBitmapFromResource(getResources(), resId, mMaxDimension, mMaxDimension);
                } catch (FileNotFoundException e) {
                    Log.e(TAG, "Can't find file to resize: " + e.getMessage());
                    FirebaseCrash.report(e);
                } catch (IOException e) {
